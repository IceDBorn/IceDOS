...

{
  ...
  environment =
    lib.mkIf (config.desktop.hyprland.enable || config.desktop.hypr) {
      ...
      etc = lib.mkIf (config.desktop.hyprland.enable || config.desktop.hypr) {
        "polkit-gnome".source =
  ...
  services = lib.mkIf (config.desktop.hyprland.enable || config.desktop.hypr) {
    dbus.enable = true;
  ...
  security = {
    polkit.enable =
      lib.mkIf (config.desktop.hyprland.enable || config.desktop.hypr) true;
