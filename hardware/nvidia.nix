...

{
	...
	systemd.services.nv-power-limit = lib.mkIf config.nvidia.enable {
		enable = true;
		...
		serviceConfig = {
			User = "root";
			ExecStart = "${config.boot.kernelPackages.nvidia_x11.bin}/bin/nvidia-smi  --power-limit=${config.nvidia.power-limit}";
		};
